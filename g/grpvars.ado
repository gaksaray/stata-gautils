...
program grpvars
    ...
    if "`fn'" == "keep" {
        ...
        _groupKeep, name(`name')
        local grouplist = r(droplist)
    }
    if "`fn'" == "clear" {
        if "`varlist'" != "" {
            noi di as err "varlist not allowed"
...
capture program drop _groupKeep
program _groupKeep, rclass
    syntax, name(namelist local)
    ...
    foreach group of global vargroups {
        if strpos("`grouplist'", "`group'") == 0 {
            global `group'
            local droplist "`droplist' `group'"
        }
    }
    return local droplist "`droplist'"
end
